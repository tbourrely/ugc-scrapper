...
pub struct Ugc {}
impl Ugc {
    pub async fn get_html_from_theaters_per_dates(
        theaters: Vec<Theater>,
        dates: Vec<NaiveDate>,
    ) -> Result<HtmlFromTheatersByDate, Error> {
        ...
        for theater in theaters.iter() {
            ...
            for date in dates.iter() {
                let html_page =
                    match Self::get_ugc_screening_page_by_theater_by_date(theater, date).await {
                        Ok(page) => page,
    ...
    async fn get_ugc_screening_page_by_theater_by_date(
        theater: &Theater,
        date: &NaiveDate,
    ) -> Result<String, Error> {
        let base_url =
